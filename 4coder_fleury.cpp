// TODO
// [X] Darken code when lister is open (like the inactive panel)
// [X] hide cursor on other panel
// [X] block cursor
...
const float COMPILATION_BUFFER_LINES = 5.0f;

int lister_open = false;
...
void custom_layer_init(Application_Links *app)
{
    ...
    // NOTE(rjf): Set up hooks.
    {
        ...
        set_custom_hook(app, HookID_SaveFile,         		 on_save_file);
    }
...
CUSTOM_DOC("Fleury startup event")
{
    ...
    //~ NOTE(rjf): Initialize panels
    {
        ...
        View_ID compilation_view = 0;
        {
            ...
            view_set_split_pixel_size(app, compilation_view, (i32)(metrics.line_height * COMPILATION_BUFFER_LINES));
    ...
    //~ NOTE(rjf): Initialize stylish fonts.
    {
        ...
        // NOTE(rjf): Small code font.
        {
            ...
            Face_Description desc = {0};
            {
                desc.font.file_name =  push_u8_stringf(scratch, "%.*sfonts/consola.ttf", string_expand(bin_path));
