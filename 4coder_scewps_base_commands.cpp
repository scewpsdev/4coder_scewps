...
function b32
is_alpha_numeric_underscore_not_newline(u8 c) {
    return (character_is_alpha_numeric(c) || c == '_') && !(c == '\n' || c == '\r');
}

function b32
is_alpha_numeric_underscore(u8 c) {
    return character_is_alpha_numeric(c) || c == '_';
}

function b32
is_whitespace_not_newline(u8 c) {
    return c == ' ' || c == '\t';
}
...
write_text(Application_Links *app, String_Const_u8 insert){
    ...
    if (insert.str != 0 && insert.size > 0){
        ...
        if (edit_success){
            ...
            if (insert.size == 1 && is_alpha_numeric_underscore_not_newline(insert.str[0])) {
                if (*menu_ptr == nullptr) { // if there is no completion list, create it
                    view_enqueue_command_function(app, view, open_completion_list);
                }
            }
            /*
            else {
                if (*menu_ptr != nullptr) { // if there is a completion list, destroy it
