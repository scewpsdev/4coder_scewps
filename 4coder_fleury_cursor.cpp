...
function void
draw_block_cursor_notepad(Application_Links *app, View_ID view_id, b32 is_active_view,
                                                 Buffer_ID buffer, Text_Layout_ID text_layout_id,
                                                 f32 roundness, f32 outline_thickness, Frame_Info frame_info){
    b32 has_highlight_range = draw_highlight_range(app, view_id, buffer, text_layout_id, roundness);
    if (!has_highlight_range){
                             ...
                             f32 roundness, f32 outline_thickness, Frame_Info frame_info)
{
    ...
    if(!has_highlight_range)
    {
        ...
        // NOTE(rjf): Draw cursor
        {
            ...
            Rect_f32 globalCursorRectCopy = global_cursor_rect;
            //globalCursorRectCopy.y0 -= 3;
            
            draw_rectangle(app, globalCursorRectCopy, roundness, ghost_color);
